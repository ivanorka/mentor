...
import { DEMO_IDENTITIES, clearDemoSession, demoSession } from "../lib/demo";
...
function SettingsContent() {
  ...
  useEffect(() => {
    ...
    apiFetch<UserData>("/users/me").then(({ data }) => { setUser(data); setName(data.name); setLocale(data.locale); }).catch(() => {
      const session = demoSession() ?? { ...DEMO_IDENTITIES[0], dashboard: "/ucenik" };
